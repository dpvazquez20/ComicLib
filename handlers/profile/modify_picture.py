...
from models.db import ComicBook

""" Modify picture handler in the profile home page
    Get: redirect to the profile home page 
    Post: it's responsible for modifying the user picture in the database"""
class ProfileModifyPictureHandler(BaseHandler):
    ...
    def post(self):
        ...
        if self.session.get('session_role') == 'admin' or self.session.get('session_role') == 'client':
            ...
            if user is not None:
                ...
                if aux is not None:
                    # Variables to be sent to the HTML page
                    values["ok_message"] = lang["picture_modified_successfully"]     # Ok message (Picture modified successfully)
                    values['session_picture'] = user.picture                         # Setting the session variable
            ...
            # Else show an error message
                ...
                values["error_message"] = lang["picture_not_modified"]     # Error message (Picture  couldn't be modified)

            del lang, picture, key, user        # Delete variables to free memory
