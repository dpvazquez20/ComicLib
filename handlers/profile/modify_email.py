...
""" Modify email handler in the profile home page
    Get: redirect to the profile home page 
    Post: it's responsible for modifying the user email in the database"""
        ...
        if self.session.get('session_role') == 'admin' or self.session.get('session_role') == 'client':
            ...
            if user is not None and len(email) > 0:
                ...
                if repeated_user.count() == 0:
                    ...
                    if aux is not None:
                        # Variables to be sent to the HTML page
                        values["ok_message"] = lang["email_modified_successfully"]       # Ok message (Email modified successfully)
            ...
            # Else show an error message
                ...
                # Values to be sent to the HTML page
                values["error_message"] = lang["email_not_modified"]     # Error message (Email couldn't be modified)
