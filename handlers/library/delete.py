...
""" Delete handler in the library home page
    Get: redirect to the library home page 
    Post: it's responsible for deleting a shelving from the database"""
class DeleteShelvingHandler(BaseHandler):
    ...
    def post(self):
        ...
        if self.session.get('session_role') == 'client':
            ...
            if shelving and shelving is not None:
                ...
                # Delete db comic
                shelving.key.delete()
                values["ok_message"] = lang["shelving_deleted_successfully"]  # Ok message (shelving deleted successfully)
                ...
                if len(aux3) > 0:
                    if not self.session.get("shelving"):
                        ...
                        comics = ComicBook.query(ComicBook.users.username == self.session.get("session_name"))  # List not empty
                        num_total = len(aux3)
                        aux3 = list()
                        for comic in comics:
                ...
                if not self.session.get("shelving"):
                    values["comics"] = self.get_comics_read_and_without_shelving(comics)
                else:
            ...
            # Else show an error message
                ...
                values["error_message"] = lang["shelving_not_deleted"]    # Error message (Shelving couldn't be deleted)
    ...
    # Delete all shelves and take out all comic books
    def delete_shelving(self, shelving, all_comics_user):
        # Update the ComicBook.users rows
        aux = list()
        for comic in all_comics_user:
            aux = list()
            for user_comic in comic.users:
                if user_comic.shelving == shelving.key:
                    user_comic.shelving = None
                aux.append(user_comic)
            comic.users = aux
            comic.put()
            del comic

        # Update the User_ComicBook rows
        user_comics = User_ComicBook.query(User_ComicBook.username == self.session.get("session_name"), User_ComicBook.shelving == shelving.key)
