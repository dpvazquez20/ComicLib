...
""" Delete handler in the library home page
    ...
    Post: it's responsible for quiting all the comics that are shown in the page"""
class QuitAllLibraryHandler(BaseHandler):
    ...
    def post(self):
        ...
        if self.session.get('session_role') == 'client':
            ...
            if aux3 and len(aux3) > 0:
                ...
                values["ok_message"] = lang["comics_quited_successfully"]  # Ok message (comics quited successfully)
            ...
            # Else show an error message
                ...
                values["error_message"] = lang["comics_not_quited"]    # Error message (comic couldn't be quited)
            ...
            del keys_page_list, aux_all_keys, all_keys, aux, aux3, all_comics_user, offset, comics,\
                lang, aux2, all_comics, shelving_name  # Delete variables to free memory
