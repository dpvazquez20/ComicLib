...
""" Main handler in the collection home page 
    Get: it's responsible for showing the collection home page with a comic list (always check if a client is logged in)
    Post: it's responsible for moving the comics list with the pagination"""
class CollectionHomeHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.session.get('session_role') == 'client':
            ...
            # Get all comics that belongs to the current user (Limited to the number given by the session variable [10 by default])
            ...
            self.session['page_name'] = "/collection"  # Current page name
            ...
            del num_total, pages_to_show, keys_page_list, all_keys, lang, comics, all_comics_user, all_comics  # Delete variables to free memory
                ...
                if num_total > 0:
                    ...
                    if page_number <= (num_pages + 1):
                        ...
                        del pages_to_show, page_number, num_elems_page, num_total, offset, \
                            ...
                            all_comics_user  # Delete variables to free memory
