...
""" Delete handler in the comics home page
    Get: redirect to the comics home page 
    Post: it's responsible for deleting a comic from the database"""
class DeleteComicHandler(BaseHandler):
    ...
    def post(self):
        ...
        if self.session.get('session_role') == 'admin':
            ...
            if comic and comic is not None:
                # Delete the comic book authors from the ComicBook_Author
                self.delete_author_comicbook_author(comic)

                # Deleting the old comic key to the comic keys lists
                all_keys.remove(comic.key.urlsafe())
                aux3.remove(comic.key)

                comic.key.delete()     # Delete db comic
                values["ok_message"] = lang["comic_deleted_successfully"]  # Ok message (comic deleted successfully)

                """I don't delete the associations between ComicBook and User because if the admin recover 
                the comic book data, the association will automatically restore"""
            ...
            # Else show an error message
                ...
                values["error_message"] = lang["comic_not_deleted"]    # Error message (Comic couldn't be deleted)

            del key, comic, keys_page_list, aux_all_keys, all_keys, aux, aux3, all_comics, offset, comics,\
    ...
    # Deleting the author in the ComicBook_Author db model
    def delete_author_comicbook_author(self, comic):
        # Deleting the author in the ComicBook_Author model
        if len(comic.authors) > 0:
            all_comic_authors = ComicBook_Author.query().fetch()
            if len(all_comic_authors) > 0:
                for comic_author1 in comic.authors:
                    for comic_author2 in all_comic_authors:
                        if comic_author1.id_aux == comic_author2.id_aux:
                            comic_author2.key.delete()
                        del comic_author2
                    del comic_author1
            del all_comic_authors
