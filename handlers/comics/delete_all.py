...
""" Delete handler in the comics home page
    ...
    Post: it's responsible for deleting all the comics that are shown in the page"""
class DeleteAllComicHandler(BaseHandler):
    ...
    def post(self):
        ...
        if self.session.get('session_role') == 'admin':
            ...
            if aux3 and len(aux3):
                # Deleting the old comic keys to the comic keys lists and delete the db comics
                for key in aux2:
                    comic = key.get()
                    self.delete_author_comicbook_author(comic)  # Delete the comicbook authors from the ComicBook_Author model
                    all_keys.remove(comic.key.urlsafe())
                    aux3.remove(comic.key)
                    comic.key.delete()     # Delete db comic
                    del key

                values["ok_message"] = lang["comics_deleted_successfully"]  # Ok message (comics deleted successfully)
            ...
            # Else show an error message
                ...
                values["error_message"] = lang["comics_not_deleted"]    # Error message (comic couldn't be deleted)

            del keys_page_list, aux_all_keys, all_keys, aux, aux3, all_comics, offset, comics,\
