...
""" Add handler in the comics home page
    Get: redirect to the comics home page 
    Post: it's responsible for adding a comic to the database"""
class AddComicHandler(BaseHandler):
        ...
        if self.session.get('session_role') == 'admin':
            ...
            if len(title) > 0 and len(type) > 0:
                ...
                if save_date != "":
                    ...
                    comic = ComicBook(isbn=isbn, title=title, publisher=publisher, edition=edition, plot=plot, type=type, origin=origin, value=value, cover=cover, save_date=save_date)
                else:
                    comic = ComicBook(isbn=isbn, title=title, publisher=publisher, edition=edition, plot=plot, type=type, origin=origin, value=value, cover=cover)
                ...
                if aux2 is not None:
                    ...
                    ###########################################################################################
                    # Variables to be sent to the HTML page
                    values["ok_message"] = lang["comic_added_successfully"]    # Ok message (comic added successfully)
                    values["keys_page_list"] = keys_page_list                  # Comics keys that are currently in the page
                    values["all_keys"] = all_keys                              # All comic keys
                    values["all_comics"] = all_comics                          # ALL comic (for the search field)
            ...
            # Else show an error message
                ...
                values["error_message"] = lang["must_insert_all_comic_attributes"]   # Error message (You must enter all comic data)
            ...
            self.response.write(jinja.render_template("/comics/default.html", **values))  # Go to the comicss home page
